        ...
        try {
            ...
            if (contentType.includes('application/json')) {
                ...
                if (data && data.success) {
                    ...
                    alert('Erro: ' + (data && data.error ? data.error : 'Resposta inválida do servidor'));
                }
            } else {
                // Server returned non-JSON (probably HTML). Show a concise message and log full body.
                const text = await response.text();
                const stripped = text.replace(/<[^>]*>?/gm, '').trim().slice(0, 300);
                alert('Erro ao cadastrar: resposta inesperada do servidor. ' + (stripped || response.statusText));
                console.error('Non-JSON response from /cadastro:', text);
            }
        } catch (error) {
