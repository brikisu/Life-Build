...
const DOM = {
  ...
  adminSidebarSection: document.getElementById('adminSidebarSection'),
  adminUserTable: document.getElementById('adminUserTable'),
  adminUserTbody: document.getElementById('adminUserTbody'),
  ...
  prefShowEmail: document.getElementById('prefShowEmail'),
  // Calendário
  ...
  customTagSelect: document.getElementById('customTagSelect'),
...
function loadData() {
  try {
    ...
    // Garantir que as etiquetas estejam sem duplicatas e existam etiquetas padrão
    normalizeAndSeedTags();
  } catch (err) {
...
// Normaliza lista de tags, remove duplicatas e adiciona etiquetas padrão se estiver vazia
function normalizeAndSeedTags() {
  // Remove duplicatas (comparação case-insensitive no nome)
  if (!Array.isArray(state.tags)) state.tags = [];
  const seen = new Set();
  state.tags = state.tags.filter(t => {
    if (!t || !t.name) return false;
    const n = String(t.name).replace('#', '').trim().toLowerCase();
    if (seen.has(n)) return false;
    seen.add(n);
    t.name = n;
    if (!t.id) t.id = `tag_${n}`;
    if (!t.color) t.color = '#4f46e5';
    return true;
  });

  // If still empty, seed from I18N default tag keys
  if (state.tags.length === 0) {
    const defaultKeys = ['tag.pessoal', 'tag.trabalho', 'tag.saude', 'tag.estudos', 'tag.geral'];
    const lang = getLang();
    const colorMap = {
      'pessoal': '#10b981',
      'trabalho': '#4f46e5',
    ...
    defaultKeys.forEach(k => {
      const label = (I18N[k] && I18N[k][lang]) ? I18N[k][lang] : (I18N[k] && I18N[k]['pt-BR']) || '';
      const name = label.replace('#', '').trim().toLowerCase();
      if (!name) return;
      state.tags.push({ id: `tag_${name}`, name: name, color: colorMap[name] || '#4f46e5' });
    });
    saveData();
  }
}
...
// NOTE: `addNewRoutine` is implemented later as an async function that
// syncs with the backend and falls back to local storage. The older
// synchronous implementation was removed to avoid duplicate definitions.
...
function openTaskDetails(taskId) {
  ...
  // Preencher select de etiqueta se existir
  if (DOM.taskTagSelect) {
    DOM.taskTagSelect.value = task.tag ? `#${task.tag}` : '';
  } else if (DOM.taskTag) {
...
function saveTaskDetails(e) {
  ...
  // prefer select value if present, otherwise free-text input
  let tagVal = '';
  if (DOM.taskTagSelect && DOM.taskTagSelect.value) tagVal = DOM.taskTagSelect.value;
  else if (DOM.taskTag) tagVal = DOM.taskTag.value;
  const tag = tagVal ? normalizeTag(tagVal) : undefined;
...
async function processAddTagForm(e) {
  if (e && e.preventDefault) e.preventDefault();
  ...
  // Adicionar etiqueta (tenta backend, depois fallback)
  try {
    await addNewTag({ name, color });
  } catch (err) {
...
async function addNewTag({ name, color }) {
  ...
  if (state.tags.some(t => t.name.toLowerCase() === normalized)) return null;

  // Try saving to backend first
  try {
    const res = await fetch(`${API_BASE_URL}/api/etiquetas`, {
      method: 'POST',
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ nome: normalized, cor: color || '#888' })
    });
    if (res.ok) {
      const j = await res.json();
      const newTag = { id: `tag${j.id}`, name: j.nome, color: j.cor };
      state.tags.push(newTag);
      saveData();
      renderTags();
      return newTag;
    }
    console.error('Failed to save tag to backend', res.status);
  } catch (err) {
    console.error('Error saving tag to backend', err);
  }

  // Fallback to local if backend fails
  const newTag = { id: `tag${Date.now()}`, name: normalized, color: color || '#888' };
  state.tags.push(newTag);
  saveData();
  renderTags();
  return newTag;
...
function renderTags() {
  ...
  // (Removed datalist usage — we use select elements only)

  // Populate tag select dropdowns (details and custom modal)
  const taskSelect = document.getElementById('taskTagSelect');
  const customSelect = document.getElementById('customTagSelect');
  // Ensure tags exist and are normalized
  if (!Array.isArray(state.tags) || state.tags.length === 0) normalizeAndSeedTags();

  [taskSelect, customSelect].forEach(sel => {
    if (!sel) return;
    sel.innerHTML = '';
    const emptyOpt = document.createElement('option');
    emptyOpt.value = '';
    emptyOpt.textContent = t('details.etiqueta') || '— Selecionar —';
...
function renderProfile() {
  ...
  // Determine display name / username from possible API keys
  const displayName = state.profile.displayName || state.profile.nome || state.profile.name || state.profile.userName || '';
  const userName = state.profile.userName || state.profile.nome || (state.profile.email ? state.profile.email.split('@')[0] : '') || '';
  const email = state.profile.email || state.profile.emailAddress || '';

  if (DOM.displayName) DOM.displayName.textContent = displayName || '';
  if (DOM.userName) DOM.userName.textContent = userName || '';
  // Show or mask email depending on persisted preference
  const showEmailPref = (state.preferences && typeof state.preferences.showEmail !== 'undefined') ? !!state.preferences.showEmail : true;
  if (DOM.userEmail) {
    if (email) {
      DOM.userEmail.value = showEmailPref ? email : maskEmail(email);
      if (DOM.showEmail) DOM.showEmail.textContent = showEmailPref ? t('profile.ocultar') : t('profile.mostrar');
    } else {
      // No email
      DOM.userEmail.value = '';
      if (DOM.showEmail) DOM.showEmail.textContent = t('profile.mostrar');
    }
  }

  // sync the settings checkbox if present
  if (DOM.prefShowEmail) {
    const v = (state.preferences && typeof state.preferences.showEmail !== 'undefined') ? !!state.preferences.showEmail : true;
    DOM.prefShowEmail.checked = v;
  }
}
...
async function enableAdminUI() {
  ...
  // ensure admin view link updates
  await loadAdminUsers();
}

async function loadAdminUsers() {
  if (!DOM.adminUserTable || !DOM.adminUserTbody || !DOM.adminNotice) return;
  DOM.adminNotice.textContent = 'Carregando usuários...';
  DOM.adminUserTable.style.display = 'none';
  try {
    const [resUsers, resBanned] = await Promise.all([
      fetch(`${API_BASE_URL}/api/admin/users`, { credentials: 'include' }),
      fetch(`${API_BASE_URL}/api/admin/banned`, { credentials: 'include' })
    ]);
    if (!resUsers.ok) {
      DOM.adminNotice.textContent = 'Não autorizado ou erro ao buscar usuários.';
      return;
    }
    const users = await resUsers.json();
    const banned = resBanned.ok ? await resBanned.json() : [];
    const bannedIds = new Set((banned || []).map(b => Number(b.user_id)));
    renderAdminUsers(users, bannedIds);
  } catch (err) {
    console.error('Erro ao carregar lista de usuários:', err);
    DOM.adminNotice.textContent = 'Erro ao carregar usuários.';
  }
}
...
function renderAdminUsers(users = [], bannedIds = new Set()) {
  ...
  if (!users || users.length === 0) {
    DOM.adminNotice.textContent = 'Nenhum usuário encontrado.';
    DOM.adminUserTable.style.display = 'none';
    return;
  }
  ...
  users.forEach(u => {
    ...
    const tdNome = document.createElement('td'); tdNome.textContent = u.nome || '';
    const tdEmail = document.createElement('td'); tdEmail.textContent = u.email || '';
    const tdAcoes = document.createElement('td');

    const banBtn = document.createElement('button');
    banBtn.textContent = bannedIds.has(Number(u.id)) ? 'Desbanir' : 'Banir';
    banBtn.className = 'btn small';
    banBtn.addEventListener('click', async () => {
      const action = bannedIds.has(Number(u.id)) ? 'unban' : 'ban';
      await adminPerformAction(action, u.id);
    });
    ...
    delBtn.addEventListener('click', async () => {
      if (!confirm(`Excluir usuário ${u.email || u.nome}? Esta ação é irreversível.`)) return;
      await adminPerformAction('delete', u.id);
    });

    tdAcoes.appendChild(banBtn);
    ...
    DOM.adminUserTbody.appendChild(tr);
  });
  DOM.adminUserTable.style.display = '';
}

async function adminPerformAction(action, userId) {
  try {
    const res = await fetch(`${API_BASE_URL}/api/admin/action`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
    ...
    if (!res.ok) {
      ...
      alert('Erro: ' + (err.error || res.statusText || 'unknown'));
      return;
    }
    await loadAdminUsers();
    ...
    alert('Erro ao executar ação administrativa. Veja console.');
  }
}
...
function maskEmail(email) {
  ...
  const visible = name[0] + '*'.repeat(Math.max(1, name.length - 2)) + name[name.length - 1];
...
function setupEventListeners() {
  ...
  if (DOM.showEmail) {
    DOM.showEmail.addEventListener('click', (e) => {
      ...
      const full = state.profile.email;
      const current = DOM.userEmail ? DOM.userEmail.value : '';
      // toggle preference and UI
      const willShow = current !== full;
      state.preferences = state.preferences || {};
      state.preferences.showEmail = willShow;
      if (DOM.prefShowEmail) DOM.prefShowEmail.checked = willShow;
      if (DOM.userEmail) DOM.userEmail.value = willShow ? full : maskEmail(full);
      DOM.showEmail.textContent = willShow ? t('profile.ocultar') : t('profile.mostrar');
      saveData();
    });
  }

  // settings checkbox listener: persist preference and update UI
  if (DOM.prefShowEmail) {
    DOM.prefShowEmail.addEventListener('change', (e) => {
      state.preferences = state.preferences || {};
      state.preferences.showEmail = !!e.target.checked;
      const email = state.profile && state.profile.email ? state.profile.email : '';
      if (DOM.userEmail) DOM.userEmail.value = state.preferences.showEmail ? email : maskEmail(email);
      if (DOM.showEmail) DOM.showEmail.textContent = state.preferences.showEmail ? t('profile.ocultar') : t('profile.mostrar');
      saveData();
    });
  }
...
function addSpecificDateFieldWithValue(dateValue) {
  ...
  // leave date input value as provided; do not enforce min
...
// Use the current origin so cookies/session are sent when frontend
// is served from the same host (avoids localhost vs 127.0.0.1 mismatch)
const API_BASE_URL = window.location.origin;

// Verificar autenticação
async function checkAuth() {
    try {
    const resp = await fetch(`${API_BASE_URL}/api/usuario`, { credentials: 'include' });
    if (!resp.ok) return false;
    const j = await resp.json().catch(() => ({}));
    return Boolean(j && j.logged_in);
    } catch (error) {
...
async function loadDataFromBackend() {
    try {
        // Load only endpoints that exist on the backend; missing endpoints should not break the whole load.
        const [rotinasResp, etiquetasResp, usuarioResp] = await Promise.all([
          fetch(`${API_BASE_URL}/api/rotinas`, { credentials: 'include' }).catch(e => ({ ok: false, json: async () => [] })),
          fetch(`${API_BASE_URL}/api/etiquetas`, { credentials: 'include' }).catch(e => ({ ok: false, json: async () => [] })),
          fetch(`${API_BASE_URL}/api/usuario`, { credentials: 'include' }).catch(e => ({ ok: false, json: async () => ({ logged_in: false }) }))
        ]);

        const rotinas = rotinasResp.ok ? await rotinasResp.json() : [];
        const etiquetas = etiquetasResp.ok ? await etiquetasResp.json() : [];
        const usuarioJson = usuarioResp.ok ? await usuarioResp.json() : { logged_in: false };
        const config = { tema: 'light', notificacoes: false, idioma: 'pt-BR', fonte: 'Inter', tamanho_fonte: 14, mostrar_concluidas: true };
        const estatisticas = {};
        ...
        state.profile = usuarioJson;

        // If the user is admin, enable admin UI and load users
        if (state.profile && state.profile.is_admin && typeof enableAdminUI === 'function') {
          enableAdminUI();
        }
...
async function saveRoutineToBackend(routine) {
    try {
        const isUpdate = !!(routine && routine.id && String(routine.id).startsWith('t'));
    const payload = {
      ...
      etiquetas: routine.tag ? [{ nome: routine.tag, cor: getTagColor(routine.tag) }] : []
    };
    if (isUpdate) payload.id = String(routine.id).replace('t', '');
    ...
    if (response.status === 401 || response.status === 403) {
      console.warn('saveRoutineToBackend unauthorized', response.status);
      return null;
    }
    ...
    try {
      ...
      console.error('saveRoutineToBackend failed', response.status, txt);
    } catch (e) {
      console.error('saveRoutineToBackend failed and could not read body', response.status);
    }
    return null;
    } catch (error) {
