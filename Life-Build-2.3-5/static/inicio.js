...
function renderProfile() {
  ...
  // Show masked email by default (click "Mostrar" will reveal)
  if (DOM.userEmail) {
    if (email) {
      DOM.userEmail.textContent = email; // show full email by default
      // Update the show/hide link to say 'Ocultar'
      if (DOM.showEmail) DOM.showEmail.textContent = t('profile.ocultar');
    } else {
...
function openCustomPeriodModal() {
  ...
  // Do not restrict date selection: allow any date (past/future)
  // Limpar formulário
  if (DOM.customPeriodForm) DOM.customPeriodForm.reset();

  // Inicializar valores padrão (vazios para permitir seleção livre)
  if (DOM.customStartDate) DOM.customStartDate.value = '';
  if (DOM.customEndDate) DOM.customEndDate.value = '';
  if (DOM.recurringStartDate) DOM.recurringStartDate.value = '';
  ...
  switch (type) {
    ...
    case 'recurring':
      if (DOM.periodRecurringSection) {
        ...
        // do not set min on recurring date inputs — allow any date
      }
      break;
  }
}
...
function addSpecificDateField() {
  ...
  // no minimum enforced for specific dates
...
function generateRoutineDates(periodType, formData) {
  ...
  try {
    switch (periodType) {
      case 'range':
        // Allow missing start/end: default to today or mirror the other value
        const start = formData.startDate ? new Date(formData.startDate + 'T00:00:00') : new Date();
        const end = formData.endDate ? new Date(formData.endDate + 'T00:00:00') : new Date(start);

        if (start > end) {
          // if start is after end, swap to be permissive
      ...
      case 'specific':
        // If specific dates not provided, fall back to startDate/endDate if present
        const rawSpecific = (formData.specificDates && formData.specificDates.length > 0)
...
// Adicionar múltiplas rotinas baseadas nas datas geradas
async function addRoutinesWithCustomPeriod(formData) {
  try {
    ...
    for (let index = 0; index < dates.length; index++) {
      ...
      try {
        ...
        const result = await addNewRoutine(routineData);
    ...
    if (errors.length > 0) console.error('Erros ao criar rotinas:', errors);
  } catch (error) {
...
async function processCustomPeriodForm(e) {
  if (e && e.preventDefault) e.preventDefault();
  ...
  // prefer select value if present
  let tagVal = '';
  if (DOM.customTagSelect && DOM.customTagSelect.value) tagVal = DOM.customTagSelect.value;
  else if (DOM.customTag) tagVal = DOM.customTag.value;
  const tag = tagVal ? normalizeTag(tagVal) : undefined;
  ...
  try {
    ...
    await addRoutinesWithCustomPeriod(formData);
    closeCustomPeriodModal();
...
function setupEventListeners() {
  ...
  if (DOM.showEmail) {
    DOM.showEmail.addEventListener('click', (e) => {
      ...
      const current = DOM.userEmail ? DOM.userEmail.textContent : '';
      if (current === full) {
        // hide
        DOM.userEmail.textContent = maskEmail(full);
        DOM.showEmail.textContent = t('profile.mostrar');
      } else {
        // show full
        DOM.userEmail.textContent = full;
...
async function loadDataFromBackend() {
    try {
        const [rotinas, etiquetas, config, estatisticas] = await Promise.all([
            fetch(`${API_BASE_URL}/api/rotinas`, { credentials: 'include' }).then(r => r.json()),
              fetch(`${API_BASE_URL}/api/etiquetas`, { credentials: 'include' }).then(r => r.json()),
              fetch(`${API_BASE_URL}/api/configuracoes`, { credentials: 'include' }).then(r => r.json()),
              fetch(`${API_BASE_URL}/api/estatisticas`, { credentials: 'include' }).then(r => r.json())
        ]);
        ...
        state.profile = await fetch(`${API_BASE_URL}/api/usuario`, { credentials: 'include' }).then(r => r.json());
